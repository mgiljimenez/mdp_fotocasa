...
# Introducción.
En los últimos años, el mercado inmobiliario se ha consolidado como uno de los sectores más competitivos y relevantes de la economía nacional.
En este contexto tan complejo y dinámico, comprender qué características influyen en el precio final de las viviendas es esencial para la toma de decisiones que afectan directamente a la gran mayoría de los ciudadanos.
Mediante la aplicación de diversas técnicas estadísticas y de aprendizaje automático estudiadas en la asignatura de MDP I, este análisis busca describir el comportamiento del mercado a la vez que se construyen modelos predictivos que permitan estimar el valor de una vivienda con base en sus características.
Para ello, a través de webscraping a fotocasa, hemos recopilado un conjunto de datos compuesto por 1867 observaciones; representando cada una de ellas un inmueble en alquiler en la ciudad de Valencia, con sus rasgos característicos. En total, cuenta con 44 variables, que describen desde aspectos de la vivienda (superficie, calidad energética, baños…) hasta cualidades del entorno urbano (precio medio del alquiler, de la venta, servicios ofrecidos por el barrio…) y del propietario.

# Análisis por componentes principales: PCA.

Como primer paso para analizar los datos que obtuvimos a través de la web de Fotocasa, se ha realizado un análisis de componentes principales (PCA). El objetivo era reducir la dimensionalidad del conjunto de datos de viviendas (manteniendo la mayor varianza posible en los datos) e identificar patrones o tendencias comunes en las características de los inmuebles ofertados.
...
Una vez seleccionadas las variables, se procede a estandarizarlas, debido a que estaban medidas en distintas unidades (como metros cuadrados, unidades, etc.). Con esta estandarización nos aseguramos que todas las variables tengan el mismo peso en el análisis, y evitar así que aquellas con magnitudes numéricas mayores influyan de forma desproporcionada en la construcción de las componentes principales.
...
Como resultado, vimos que empleando este método (varianza explicada acumulada), con 8 componentes se explica el 72% de la variabilidad total de nuestros datos, sin embargo, seleccionar 8 componentes complicaría excesivamente la interpretación de los resultados, por lo que estudiamos otros criterios para seleccionar el número de componentes adecuado. Empleamos el método del codo, y obtuvimos que podríamos seleccionar 4 componentes principales, por tanto nos quedamos con esta última cifra para posteriormente facilitar el análisis de los resultados.

## Análisis de atípicos.
Tras la seleccion de componentes, procedemos a evaluar primeramente la presencia de observaciones atípicas extremas dentro del espacio generado por las componentes principales. Para ello, utilizamos el estadístico T² de Hotelling, que nos permite medir la distancia de cada observación al centro de la nube de puntos proyectada sobre las primeras 4 componentes. 
...
Por tanto, se asume que el resto de observaciones con distancias elevadas no se corresponden a errores de codificación ni valores anómalos injustificados, sino que representan casos reales y válidos. Por ello, no se ha considerado necesario excluirlas del análisis, ya que la técnica de PCA captura las tendencias generales del conjunto, pero es esperable que algunas viviendas reales, aunque menos frecuentes, se representen peor o no tengan valores similares al resto.
...
```
A partir de los resultados obtenidos se han extraído las siguientes conclusiones:
...
```{r tabla, results='asis', fig.align='center', echo=FALSE}
fotocasa = read_excel("fotocasaImp.xlsx")
...
El paso previo a la clasificación fue comprobar si existen tendencias de agrupación en los pisos basándonos en las variables del cluster. Para ello, se ha elaboró un *heatmap*. Dado que pretendemos identificar similitudes entre las infraestructuras ofrecidas por cada ubicación, consideramos adecuado aplicar una medida de cercanía.
Probamos todas las distancias para variables numéricas disponibles y, finalmente, se obtiene una tendencia de agrupación más compacta con la euclídea.
...
A continuación, estudiamos qué variables tienen más peso en la nueva clasificación a través de un Análisis de Componentes Principales con 2 dimensiones. Asimismo, utilizamos las columnas cluster y priceAmount como auxiliares para entender su relación con el resto de variables consideradas.

```{r fig.align='center', fig.height=3, fig.width=4, include=FALSE, echo=FALSE}
...
```{r pcaPLOTS, fig.align='center', fig.height=3, fig.width=7, echo=FALSE}
...
```{r mediasServicios, fig.align='center', fig.height=2.2, fig.width=6, echo=FALSE}
...
Finalmente, para visualizar geográficamente los resultados obtenidos, hemos representado los 7 clusters en un mapa de la ciudad de Valencia, utilizando las variables latitud y longitud. Así, ha sido posible determinar a qué zona corresponde cada uno de los grupos.
...
```{r fig.align='center', out.width="0.3\\linewidth", echo=FALSE}
...
```{r clasificacion_final, fig.align='center', fig.height=1.5, fig.width=7, echo=FALSE}
# Datos base
...
Tras comparar los anteriores gráficos con el mapa de Valencia, y ver a que zona pertenece cada grupo, se han extraído las siguientes conclusiones:
...
```{r map_subtipo_mem, include=FALSE, echo=FALSE}
...
```{r trans_precio_mem, include=FALSE, echo=FALSE}
...
```{r Preparación de los Datos, include=FALSE, echo=FALSE}
...
En la primera fase del análisis, se han seleccionado la mayoría de variables disponibles (incluyendo el cluster por servicios diseñado en análisis previos); descartando aquellas cuya información resultaba inservible o redundante.
Para evitar el diseño de un modelo sesgado, escalamos las variables numéricas y dividimos los datos en los conjuntos de entrenamiento (70%) y test (30%). Además, realizamos un undersampling en la base de entrenamiento eliminando aleatoriamente parte de los pisos sin bajada de precio; puesto que en la base original hay un fuerte desbalanceo que podría favorecer sistemáticamente las predicciones de la clase predominante.
...
```
Seguidamente, se ha propuesto el modelo inicial. Aplicamos la validación cruzada con 5 grupos o folds y 10 repeticiones hasta quedarnos con 1 única componente principal, minimizando así el error de predicción.
...
Hemos seleccionado esta métrica como medida de calidad porque tiene en cuenta los aciertos en ambas clases de pisos (0, 1), sin proporcionar resultados engañosos ni dejarse condicionar por el desbalanceo.
Esta primera versión muestra una capacidad predictiva aceptable, generando mejores resultados que los que se obtendrían por azar.
...
A continuación, intentaremos incrementar la eficacia mediante un test de independencia.
El objetivo es proponer un nuevo modelo formado por las variables que presentaran diferencias significativas entre clases. Así, descartamos la inclusión de variables sin capacidad predictiva, que únicamente generan ruido y disminuyen la fiabilidad de las predicciones.
...
Una vez realizada la selección, volvemos a dividir, escalar y reducir la nueva base de datos y generamos el segundo modelo; compuesto nuevamente por una sola componente principal.
...
ggplot(df, aes(x = Score, fill = Clase)) +
  ...
  theme_minimal()
```
Aunque se obtiene una distribución relativamente uniforme, se maximiza una vez más la exactitud balanceada a través de la curva ROC:
...
```{r, echo=FALSE, fig.width=5, fig.height=3.5, out.width="80%", fig.align='center'}
...
```{r selComp2, echo = FALSE, message = FALSE, warning=FALSE, echo=FALSE, fig.align='center'}
A <- 4
invisible({
  pdf(file = NULL)
...
```{r topVar, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
...
En resumen, este proyecto nos permitió entender mucho mejor cómo funciona el mercado de alquiler en Valencia. A partir de un conjunto de casi 1.900 viviendas obtenidas con web scraping, aplicamos diferentes técnicas para analizar, clasificar y predecir precios.

Pudimos identificar qué factores influyen más en el precio, cómo se agrupan los inmuebles según su entorno y qué combinaciones de características están relacionadas con precios altos o bajos. Además, construimos modelos que nos ayudan tanto a estimar el precio de alquiler como a predecir si un piso bajará de precio, con resultados bastante aceptables.

En general, creemos que el análisis fue útil no solo para describir el mercado, sino también para generar herramientas que podrían aplicarse en estudios similares, en estrategias comerciales o incluso para tomar decisiones de inversión.
...
```{r pca_confort_GRAFICO, fig.align='center', fig.height=3, fig.width=7}
...
```{r niveles_calida, fig.align='center', fig.height=1.5, fig.width=6}
...
```{r guardarresul, fig.align='center', fig.width=4, fig.height=2}
library(dplyr)
...
```{r trans_muni_preci, echo=FALSE, fig.align='center', fig.height=4, fig.width=8}
...
```{r num_dimensions, echo=FALSE, fig.align='center', fig.height=4, fig.width=4}
...
```{r municipio, echo=FALSE, fig.align='center', fig.height=3, fig.width=6}
...
```{r Grafico Dispersio, fig.align='center', fig.pos='H', fig.width=7, fig.height=3, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r Coordenadas Paralela, fig.width=5, fig.height=2.5, fig.align='center', fig.pos='H', echo=FALSE, warning=FALSE, message=FALSE}
...
```{r selección_variable,message=FALSE, warning=FALSE,echo=FALSE}
...
```{r undersamplin,message=FALSE, warning=FALSE,echo=FALSE}
...
```{r separacion_dato,message=FALSE, warning=FALSE,echo=FALSE}
set.seed(100)
...
```{r train_tes,message=FALSE, warning=FALSE,echo=FALSE}
...
```{r kfold,message=FALSE, warning=FALSE,echo=FALSE}
...
```{r modelo_ld, warning=FALSE,echo=FALSE}
set.seed(100)
...
Y <- variables$priceAmount  # Variable objetivo
```
Escalamos las matrices y creamos el modelo con las componentes calculadas.
...
```
Los gráficos muestran un resumen del modelo, que analizaremos más en profundidad más adelante. Muestra que cuenta con una buena capacidad predictiva, con un Q² mayor a 0.5, es decir, explica más de un 50% de la variabilidad de la variable estudiada, PriceAmount, así como un error aceptable.

#### Variables por componente

Una vez calculados los componentes, para conocer mejor el modelo debemos conocer las 5 variables con más peso por cada componente.
```{r, echo=FALSE}
# Obtener pesos estandarizados (loadings) de X
pesos <- mypls@loadingMN
...
# Mostrar variables más influyentes por componente
top_vars_por_componente
```

#### Atípicos
```{r, echo=FALSE, fig.width=4, fig.height=4}
## Creamos una variable ficticia para colorear el gráfico
proced <- factor(rep("all", nrow(X)))  # Todos en un mismo grupo

## Gráfico de scores con elipse T² (Hotelling)
plot(x = mypls, typeVc = "x-score",
     ...
     parCexMetricN = NA)
```
Al analizar el gráfico de scores con la elipse de Hotelling T2, se observa que la mayoría de las observaciones se encuentran dentro del límite esperado, lo que indica un comportamiento multivariado normal. Sin embargo, algunas observaciones se sitúan fuera de la elipse, lo que sugiere la posible presencia de valores atípicos.
A continuación, vamos a calcular el estadístico T2-Hotelling para cada observación de la base de datos, a partir de sus scores y varianza, con el objetivo de establecer unos umbrales que nos permitan detectar outliers extremos.
```{r Umbral 95, echo=FALSE}
...
```{r Umbral 99, echo=FALSE}
F99 = A*(N**2 - 1)/(N*(N - A)) * qf(0.99, A, N-A); F99
```

```{r, echo=FALSE}
plot(as.numeric(rownames(X)), miT2, type = "l",
     xlab = "Observaciones (líneas del Excel)", ylab = "T²",
     main = "PLS: T² de Hotelling",
     ylim = c(0, max(miT2, F99) + 1))

abline(h = F95, col = "orange", lty = 2, lwd = 2)
abline(h = F99, col = "red3", lty = 2, lwd = 2)

```
En el gráfico anterior se muestra el valor del estadístico T² de Hotelling para cada observación, junto con los umbrales críticos del 95% (línea naranja discontinua) y del 99% (línea roja discontinua). Estas líneas marcan los límites de confianza para la detección de posibles valores extremos, que tienen el valor de 9.52 y 13.35 respectivamente.

Podemos observar que existen varias observaciones que superan dichos umbrales, lo que indica que podrían ser consideradas atípicas extremas en el espacio de componentes latentes. Estas observaciones se alejan significativamente del centro del modelo, lo cual podría reflejar comportamientos anómalos o patrones particulares en los datos.

Para complementar este análisis de distancias, a continuación se utilizará otra métrica que evalúa el grado de ajuste del modelo para cada observación individual, mediante la suma de cuadrados de los residuos. Se han establecido de nuevo unos umbrales, del 99% y 95%, mediante los cuales podemos detectar outliers moderados.
```{r, echo=FALSE}
# Cálculo de la SCR (Suma de Cuadrados Residual)
myT <- mypls@scoreMN
myP <- mypls@loadingMN
myE <- scale(X) - myT %*% t(myP)
mySCR <- rowSums(myE^2)  # Sum of Squared Residuals (SPE)

# Parámetros para los límites de confianza
g <- var(mySCR) / (2 * mean(mySCR))
h <- (2 * mean(mySCR)^2) / var(mySCR)
chi2lim <- g * qchisq(0.95, df = h)
chi2lim99 <- g * qchisq(0.99, df = h)

# Gráfico adaptado: eje x = número de fila del Excel
plot(as.numeric(rownames(X)), mySCR, type = "l", 
     main = "PLS: Distancia² al modelo", 
     ylab = "SCR", xlab = "Observaciones (líneas del Excel)",
     ylim = c(0, max(mySCR, chi2lim99) + 0.5))

abline(h = chi2lim, col = "orange", lty = 2)
abline(h = chi2lim99, col = "red3", lty = 2)

```
De nuevo aparecen observaciones que presentan desviaciones notables, esta vez en cuanto a su distancia al modelo. La SCR nos ayuda a detectar datos que, si bien no están alejados en el espacio de componentes, están mal representados por el modelo. Vamos a identificar cuáles son estos valores atípicos para valorar su impacto en el análisis.
En la siguiente tabla se resumen las observaciones que superan los umbrales de confianza del 95% y 99%, tanto para T² como para SCR. Este cruce nos permite distinguir entre outliers estructurales (por T2), de mal ajuste (por SCR), o ambas cosas.
```{r, echo=FALSE}
# Identificar outliers por T2
outliers_t2_95 <- which(miT2 > F95)
outliers_t2_99 <- which(miT2 > F99)

# Identificar outliers por SCR
outliers_scr_95 <- which(mySCR > chi2lim)
outliers_scr_99 <- which(mySCR > chi2lim99)

# Unir y cruzar
unique_outliers <- sort(unique(c(outliers_t2_95, outliers_scr_95)))
outlier_table <- data.frame(
  Obs = unique_outliers,
  T2 = miT2[unique_outliers],
  SCR = mySCR[unique_outliers],
  T2_95 = miT2[unique_outliers] > F95,
  T2_99 = miT2[unique_outliers] > F99,
  SCR_95 = mySCR[unique_outliers] > chi2lim,
  SCR_99 = mySCR[unique_outliers] > chi2lim99
)

# Ordenar por SCR o T2 si quieres:
outlier_table <- outlier_table[order(-outlier_table$SCR), ]
head(outlier_table, 10)  # Ver los 10 primeros outliers
...
```
Se observa que las variables surface, bathrooms y rooms presentan una fuerte contribución al modelo, al situarse alejadas del origen. Por el contrario, otras variables como energyEfficiencyRating o tieneAscensor se encuentran próximas al centro del gráfico, lo que sugiere una menor aportación a la varianza explicada por las componentes principales. Esta representación refuerza la selección previa de variables significativas basada en el análisis VIP y los valores-p, al destacar visualmente aquellas que más estructura aportan al modelo.

A continuación, se representa un gráfico de pesos conjuntos (w*c) de las variables predictoras (X) y la variable de respuesta (Y) sobre los dos primeros componentes extraídos mediante el modelo PLS. Este gráfico permite visualizar las relaciones y asociaciones entre variables en el espacio latente común.
```{r, echo=FALSE, fig.width=4, fig.height=4}
library(ggplot2)
library(ggrepel)

# Preparar los datos de weights y c
# mypls@weightStarMN: pesos estandarizados para X (variables predictoras)
# mypls@cMN: pesos para Y (variables respuesta)
data_plot <- rbind(
  data.frame(mypls@weightStarMN, space = "X"),
  data.frame(mypls@cMN, space = "Y")
)

# Añadir columna con el nombre de las variables
data_plot$variable <- rownames(data_plot)

# Graficar con ggplot2 y ggrepel para evitar superposición de etiquetas
ggplot(data_plot, aes(x = p1, y = p2, color = space, shape = space)) +
  geom_point(size = 3) +
  ...
  labs(
    ...
    color = "Espacio",
    shape = "Espacio",
    title = "Gráfico de weights personalizado (componentes 1 y 2)"
  )

```
En el gráfico se observa cómo las variables predictoras (X, en rojo) y la variable de respuesta (Y, en azul) se proyectan sobre los dos primeros componentes. La proximidad entre la variable de respuesta y1 y algunas variables predictoras sugiere una mayor contribución de estas últimas a la explicación del comportamiento de y1. Por ejemplo, variables como surface, bathrooms, priceDescription_buy o propertyCounter_buy muestran pesos relativamente altos en el mismo cuadrante que y1, indicando una posible asociación positiva en el espacio latente. En cambio, otras variables más alejadas del punto y1 o próximas al origen contribuyen en menor medida al modelo en estos dos primeros componentes.

#### Modelo ajustado
Esta interpretación puede reforzarse mediante el análisis de los coeficientes de regresión del modelo PLS. En el gráfico de barras que se muestra a continuación, se representan los coeficientes asociados a cada variable predictora, ordenados por su influencia (en valor absoluto) en la predicción. De esta manera, es posible comprobar si las variables que se situaban próximas a la variable de respuesta en el espacio de componentes efectivamente tienen un peso relevante en el modelo, lo que valida la coherencia entre la representación geométrica y la interpretación estadística.
```{r, echo=FALSE}
...
```
Este análisis de coeficientes complementa la interpretación de los pesos (weights), ya que mientras las componentes nos muestran la estructura latente compartida entre X e Y, los coeficientes reflejan cómo esa estructura se traduce en una predicción directa. Las variables que ocupaban posiciones alejadas del origen en el gráfico de weights tienden a coincidir con las de mayor magnitud de coeficiente, confirmando su importancia en el modelo.

Sin embargo, el hecho de que una variable tenga un coeficiente alto no garantiza que sea significativa. Para evaluar si las variables aportan información relevante, se lleva a cabo un análisis de significancia estadística mediante p-valores.
```{r, echo=FALSE}
...
pvalores = pvalJack(pls = myplsC, X = X, Y = Y)
round(pvalores, 4)
```
Observamos que, al tomar como referencia un nivel de significación del 5%, varias variables no resultan estadísticamente significativas. Por tanto, procedemos a realizar un filtrado adicional mediante el método VIP (Variable Importance in Projection), el cual permite ordenar las variables según su contribución al modelo. Este enfoque nos ayuda a identificar y retener únicamente aquellas variables que aportan mayor información al modelo PLS. Este análisis complementario mediante el método VIP refuerza la evaluación de los coeficientes, permitiendo discriminar de forma más robusta las variables verdaderamente relevantes.
```{r, echo=FALSE}
barplot(sort(myplsC@vipVn, decreasing = TRUE), 
        main = "VIP (Variable Importance in Projection)", 
        las = 2)
abline(h = 1, col = "red", lty = 2)

```
En el gráfico de valores VIP, las variables con un valor superior a 1 —umbral comúnmente aceptado para considerar una variable como relevante— son: surface, bathrooms, priceDescription_buy, rooms, y, muy cerca del umbral, priceDescription_rent. Estas variables pueden considerarse las más importantes dentro del modelo actual.

Procedemos a crear un nuevo modelo con las variables con valores VIP que superan el umbral y a compararlo con el obtenido, para comprobar cuál de los dos modelos se ajusta mejor a los resultados esperados.
```{r, echo=FALSE}
vip_vals <- myplsC@vipVn
vip_filtradas <- sort(vip_vals[vip_vals > 1], decreasing = TRUE)
vip_filtradas

```
```{r, echo=FALSE}
X_reducido <- X[, c("surface", "bathrooms", "priceDescription_buy", "rooms")]
mypls_reducido <- opls(X_reducido, Y,
                       predI = 1,
                       scaleC = "standard",
                       crossvalI = 10,
                       info.txtC = "none",
...
# Modelo completo
Y_pred_completo <- fitted(myplsC)
R2_completo <- 1 - sum((Y - Y_pred_completo)^2) / sum((Y - mean(Y))^2)

# Modelo reducido
Y_pred_reducido <- fitted(mypls_reducido)
R2_reducido <- 1 - sum((Y - Y_pred_reducido)^2) / sum((Y - mean(Y))^2)

cat("R2 completo:", round(R2_completo, 4), "\n")
cat("R2 reducido:", round(R2_reducido, 4), "\n")

```
```{r, echo=FALSE}
rmse <- function(obs, pred) sqrt(mean((obs - pred)^2))

rmse_completo <- rmse(Y, Y_pred_completo)
rmse_reducido <- rmse(Y, Y_pred_reducido)

cat("RMSE completo:", round(rmse_completo, 4), "\n")
cat("RMSE reducido:", round(rmse_reducido, 4), "\n")

```
Para comprobar la efectividad del nuevo modelo, se va a comparar con el original para poder apreciar la diferencia, en caso de existir, en la capacidad predictiva.
```{r, echo=FALSE}
library(ggplot2)

df_plot <- data.frame(
  Real = as.numeric(Y),
  Completo = as.numeric(Y_pred_completo),
  Reducido = as.numeric(Y_pred_reducido)
)

ggplot(df_plot, aes(x = Real)) +
  geom_point(aes(y = Completo, color = "Completo"), alpha = 0.7) +
  geom_point(aes(y = Reducido, color = "Reducido"), alpha = 0.7) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(title = "Comparación de predicciones: modelo completo vs reducido",
       y = "Predicción",
       color = "Modelo") +
  theme_minimal()
...
En la gráfica se observa la dispersión de las predicciones frente a los valores reales para ambos modelos: el modelo completo (en rojo) y el modelo reducido (en azul), este último construido únicamente con las variables que resultaron significativas según los análisis de coeficientes y valores VIP.
