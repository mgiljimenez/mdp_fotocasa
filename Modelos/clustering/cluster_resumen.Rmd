...
.
```{r heatmap, fig.align='center', fig.height=2, fig.width=3}
...
ggplot(clasificacion, aes(x = 1, y = reorder(Zona, Cluster), fill = Cluster)) +
  geom_tile(width = 0.8, height = 0.8, aes(fill = Cluster), show.legend = FALSE) +  # ⬅ más ancho
  geom_text(aes(label = Cluster), color = "white", fontface = "bold", size = 4) +    # mejor tamaño
  geom_text(aes(x = 2, label = Zona), color = "black", hjust = 0, size = 4) +        # texto desplazado
